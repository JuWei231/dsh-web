...
def track_frames(sprites, mode, i):
    ...
    if mode == 'idle':
        sc = [1.0, 1.012, 1.005, 1.010, 1.005, 1.0][i]
        ro = [0.0, -0.7, 0.0, 0.7, 0.0, 0.0][i]
        return render_cell(perch, pa, scale=sc, rot=ro)
    ...
    if mode == 'jumping':
        # natural flit: one flying sprite rides a smooth arc (no sprite swaps)
        fly, fa = sprites['flying']
        dy = [20, 10, -4, 6, 16][i]
        ro = [0, -4, -8, -4, 0][i]
        sc = [0.95, 1.0, 1.05, 1.0, 0.95][i]
        return render_cell(fly, fa, scale=sc, rot=ro, dy=dy)
    if mode == 'failed':
