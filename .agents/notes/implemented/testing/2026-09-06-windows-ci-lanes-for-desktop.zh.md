...
- `ci.yml` 新增 `desktop-windows` job 在 `windows-latest` 上跑同一套测试，真实创建 NTFS junction、走真实 Windows 路径语义（POSIX 上只是模拟）。
...
- workflow 步骤本身在下次 tag/dispatch 运行时自证；其启动流程已按上文描述在本地端到端演练过。
