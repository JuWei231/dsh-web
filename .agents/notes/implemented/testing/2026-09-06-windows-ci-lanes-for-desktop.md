...
- A `desktop-windows` job in `ci.yml` runs the same suite on `windows-latest`, exercising real NTFS junction creation and Windows path semantics that the POSIX runs only simulate.
...
- The workflow steps themselves prove out on their next tag/dispatch run; the boot flow they run was exercised locally end to end as described above.
