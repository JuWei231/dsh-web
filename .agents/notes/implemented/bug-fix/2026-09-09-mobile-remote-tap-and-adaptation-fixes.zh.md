...
6. **头部动作：入座才隐藏，对齐靠测量。** 隐藏规则挂在 `body.dsh-remote-header-seated` 上，只有 `seatHeaderActions()` 确认动作已真正位于标签行后才设置；`alignActionsText()` 每个 tick 测量头部右侧内容边与动作右边缘之差，把标签行的 `margin-right` 收敛过去，静态值只作首帧猜测。
...
- 包内测试：`pnpm --filter @linxin666/dsh-remote-web-ui test` 355 个用例通过，其中 4 个新增用例分别覆盖"嵌套 `_frame` 不算应用框架""回车只在输入区改写""带修饰类的行仍被抑制且退出时还原""头部隐藏规则受入座门控"，另有重写的切换顺序用例。
