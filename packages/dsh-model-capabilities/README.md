...
The plugin has no settings namespace of its own: it edits the official `llm-pi-ai` namespace (the same document the Models page and `settings.yaml` manage), so there is nothing to configure and nothing to migrate. All state lives in the provider profiles you already see on the Models page.
